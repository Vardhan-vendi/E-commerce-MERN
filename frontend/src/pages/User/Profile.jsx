...
const Profile = () => {
  ...
  const [updateCurrentProfile, { isLoading }] = useProfileMutation();
  ...
  const updateHandler = async (e) => {
    ...
    try {
      ...
      const res = await updateCurrentProfile(updateData).unwrap();
      dispatch(setCredentials(res));
      toast.success("Profile updated Successfully");
    } catch (err) {
      toast.error(err?.data?.message || err.message || "updation failed");
      triggerShake();
    }
  };
  ...
  return (
    <div className="min-h-screen w-full relative flex items-center justify-center bg-[#090d16] overflow-hidden font-sans py-12">
      ...
      <div className="relative w-full max-w-[28rem] mx-4 sm:mx-0 z-10 p-[1px] rounded-2xl overflow-hidden bg-gradient-to-b from-white/10 to-transparent">
        <div
          className={`w-full h-full glass-card p-6 sm:p-10 rounded-[15px] flex flex-col ${shake ? "animate-shake" : ""}`}
        >
          ...
          <form className="flex flex-col space-y-6" onSubmit={updateHandler}>
            {/* Username Input */}
              ...
              <span className="absolute left-3 text-slate-400 group-focus-within:text-violet-400 transition-colors duration-300">
                ...
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M16.5 12a4.5 4.5 0 11-9 0 4.5 4.5 0 019 0zm0 0c0 1.38 1.12 2.5 2.5 2.5s2.5-1.12 2.5-2.5V8.25a3.75 3.75 0 00-3.75-3.75h-1.5a8.25 8.25 0 10-2.38 15.631"
                  />
                </svg>
              </span>
              <input
